...
try {
	var __IntegralASConfig = {
		...
		protocol: 'https',
		jsref: "",
		asid: "2518f5a1-ed3a-11ea-a314-024ba0cfe8f4",
...
a)});A.once("requestBootstrapper",function(a){var b=l.forcedBootstrapperHost||"http://jsconfig.adsafeprotected.com";"string"===typeof a&&(a=JSON.parse(a));var c=a&&a.config,s=a&&a.sca,n=a&&a.xsca;c&&A.trigger("jsonp",b+"/jsconfig"+c+(l.adSessionId?"&adSessionId="+l.adSessionId:""),function(a){Object.assign(l,"string"===typeof a?JSON.parse(a):a);l.isResolved=!0;l.sp_cdnScripts={sca:s,xsca:n};A.trigger("callContinueMain")},!1,"cbName")});a=[{environment:new aa(a),adSessionReadyStrategy:new Va(a)},{environment:new Wa,
...
c=0,d=l.useFIF&&u.friendlyIframe||u.isSpecifiedAd(),e=function(e){var s=t.isNodeXDomainIframe(e)||t.isNodeCreative(e),n=!e;g=n?l.contextNode.parentNode:e;if(s||n)m.getAggregator().trigger("evaluateCreativeFinderResult",e);else{if(d){var q;q=s=e;n=u.friendly?p.getTop().document.body:l.contextNode.ownerDocument.body;for(;3>c;){if(n&&s===n){q=s;c=3;break}t.isWindow(s)&&(q=t.crossQuerySelector("["+k.AD_IDENTIFIER+"-"+l.asid+"]"),s=f.isDef(q)?q:s);q=s=f.isDef(s)?s.parentNode||s:s;c++}s=q}else s=e;if(t.isWindow(s)){var r;
try{r=s.document}catch(z){r=l.contextNode.ownerDocument}e=r}else e="IFRAME"===t.getNodeName(s)?e.contentWindow.document:s;r=[];var n=(s=e&&e.getElementsByTagName&&e.getElementsByTagName("*"))&&s.length,B;if(s)for(var w=0;w<n;w++)B=(q=s[w])&&q.nodeName&&q.nodeName.toUpperCase(),!q.children||0!==q.children.length||f(b).contains(B)||f(a).contains(B)||f(r).contains(B)||r.push(B);r=b.concat(r);(new Xb(r)).searchDescendants(e)}};m.getAggregator().on("creativeFinderBatchCompleted",e);return{find:e,getClosestContainerName:function(){return t.getNodeName(g)}}},
Xb=function(g){var a=function(a){function c(a){var b=t.getNodeName(a),c=t.isNodeCreative(a)||"IFRAME"===b;b&&c&&!t.isEmptyTeadsFrame(a)&&(b=t.getNodeArea(a),b>s&&(h=a,s=b))}function d(){do c(a[e]),e+=1;while(0!==e%n&&e<a.length);e<a.length?p.execAtEndOfThread(d,0):m.getAggregator().trigger("creativeFinderBatchCompleted",h)}var e=0,h=null,s=-1,n=50;if(a.length&&m.getAggregator().request("useYieldSearch"))d();else{for(e;e<a.length;e++)c(a[e]);m.getAggregator().trigger("creativeFinderBatchCompleted",
h)}};return{searchDescendants:function(b){b=b||l.contextNode.parentNode;var c,d,e=[];for(c=0;c<g.length;c++){d=g[c].toLowerCase();var h=b.getElementsByTagName&&b.getElementsByTagName(d);if(h&&h.length)for(d=0;d<h.length;d++)e.push(h[d])}a(e)}}},Kb=function(){var g,a=!1,b=!1,c=!1,d={status:k.RENDER.STATUS.OTHER,details:k.RENDER.DETAILS.OTHER},e={status:k.RENDER.STATUS.DETECTED,details:k.RENDER.DETAILS.ENVIRONMENT},h=m.getAggregator().request("omidAdSessionContext"),s=function(){c=a=!0},n=function(a){a&&
(a.status=k.RENDER.STATUS.DETECTED);m.getAggregator().trigger("adRendered")},q=function(){var a=!1;u.isOmid()&&h&&h.useOMID13Logic&&(a=!0);return a},f=function(a){var b={},c=p.getDoc(),d=function(e){var h=e&&e.type;if("readystatechange"===h&&"complete"===e.target.readyState||"load"===h)n(b),g.saveNode(a),"readystatechange"===h?g.save(k.RENDER.DIAGNOSTIC.DOCUMENT_READY):"load"===h&&g.save(k.RENDER.DIAGNOSTIC.LOAD_FIRED),w(b),y.removeEvent(a,"load",d),y.removeEvent(c,"readystatechange",d)};"complete"===
c.readyState?(n(b),g.saveNode(a),g.save(k.RENDER.DIAGNOSTIC.DOCUMENT_READY)):(y.addEvent(a,"load",d),y.addEvent(c,"readystatechange",d));return b},z=function(){var a=p.getWindow().mraid,b=a&&a.getState&&a.getState(),c=function(){y.removeEvent(a,"ready",c);g.save(k.RENDER.DETAILS.ENVIRONMENT);g.save(k.RENDER.DIAGNOSTIC.MRAID_READY);n();w({status:k.RENDER.STATUS.DETECTED})};g.save(k.RENDER.DETAILS.ENVIRONMENT);"default"===b?(n(),g.save(k.RENDER.DIAGNOSTIC.MRAID_DEFAULT),d.status=k.RENDER.STATUS.DETECTED):
"loading"===b?(y.addEvent(a,"ready",c),g.save(k.RENDER.DIAGNOSTIC.MRAID_LOADING)):(y.addEvent(a,"ready",c),g.save(k.RENDER.DIAGNOSTIC.MRAID_UNKNOWN));w(d)},B=function(a){a===k.RENDER.STATUS.DETECTED&&m.getAggregator().trigger("perfCheckpoint",{type:k.PERF_TIMELINE_TRACKER_CONFIG.RENDER.TYPE,timestamp:p.now(),code:k.PERF_TIMELINE_TRACKER_CONFIG.RENDER.CODE})},w=function(b){var c=g.build();b=b.status||d.status;c=c?c:d.details;a&&b===k.RENDER.STATUS.DETECTED?(B(b),m.getAggregator().trigger("addThrottledOutputItem",
"rend",b),m.getAggregator().trigger("addThrottledOutputItem","renddet",c)):(B(b),m.getAggregator().trigger("addOutputItem",{output:b},"rend"),m.getAggregator().trigger("addOutputItem",{output:c},"renddet"))},v=function(){m.getAggregator().on("primaryadfound",function(a){var b,c=d,e=new Yb,h=!0;m.getAggregator().on("evaluateCreativeFinderResult",function(a){if(t.isNodeXDomainIframe(a))b?p.clearInterval(b):h=!1,c=f(a);else if(a){var d=!1,s=t.getRect(a);a&&"IMG"===t.getNodeName(a)&&(d=0===a.naturalWidth||
0===a.naturalHeight);!d&&s.width>=k.RENDER.WIDTH_THRESHOLD&&s.height>=k.RENDER.HEIGHT_THRESHOLD&&(n(c),b?p.clearInterval(b):h=!1);g.saveNode(a)}else a=e.getClosestContainerName()||g.getAdNodeName(a)||c.details,g.save(a);w(c)});var s=function(){var b=a&&a.getAdNode();try{e.find(b)}catch(c){m.getAggregator().trigger("renderdiag",c)}},q=m.getAggregator().request("mobileApp");q&&q.isMobileAppEnvironment&&q.isMobileAppEnvironment()&&u.isMraid()?z():(s(),h&&(b=p.setInterval(s,500)));m.getAggregator().trigger("eligiblerender")})},
...
d=g[a],e=f(arguments).toArray();e.shift();f.isUndef(d)||(c=f.isFunction(d)?d.apply({},e):d);return c},provide:function(b,c){c?g[b]=c:f(b).each(a);return c}}},Eb=function(){return{createInstance:function(g,a,b,c){return u.isDomless()?new cc:new dc(g,a,b,c)}}},dc=function(g,a,b,c){var d;return{collect:function(){var a=g.find(),h=a.getOutOfViewReasons(),s=b.isHidden(),n=c.isDeviceTypeGroupMobile()&&d?d:E(t.calcWinDims()),q=a.getDims();d=n;if(!a.hasAd()||!n.hasValidDims()||!q.hasValidDims())return{viewState:k.NA,
...
tabHidden:f.stringifyTriState(s),posViewState:a.getViewState(),adCompCount:a.getComponentCount(),sliceStatus:a.getSliceStatus()}}}},cc=function(){return{collect:function(){return{}}}},ec=function(g){g=g||f.identity;var a=[],b=p.now(),c=function(){var c=p.now();a.length&&(a[a.length-1].duration+=c-b,b=c)};return{clear:function(){c();a=[]},fastForward:c,get:function(){return a},hasAlwaysBeen:function(b){return 1===a.length&&a[0].state===b},addState:function(b){var e=a[a.length-1];b=g(b);c();0!==a.length&&
b===e.state||a.push({state:b,duration:0})}}},za=function(g,a){var b=g||1,c=0,d=0,e,h=0;return{start:function(){0===c%b&&(e=p.now())},stop:function(){var s=e||a;0===c%b&&(h+=p.now()-s,d++);c++},getTime:function(){return h},getCount:function(){return d}}},Aa=function(){var g,a=0,b=0,c=new D(!0),d=function(){g&&(a++,c.trigger(a),b>a?p.execAtEndOfThread(d):e())},e=function(){g=!1;a=0};return{onTick:function(a,d){var e=f.collapseArgsIntoHash(a,d);f(e).each(function(a,c){a=parseInt(a);b=a>b?a:b});c.on(e)},
start:function(){g||(g=!0,p.execAtEndOfThread(d))},kill:e,isActive:function(){return g}}},fc=function(g,a){var b,c=a,d=function(a){b||(a&&g(),b=p.setInterval(g,c))},e=function(a){a&&g();p.clearInterval(b);b=null};return{start:d,stop:e,updateFrequency:function(a,b){c=a;e();d(b)}}},X=function(g){var a,b,c=0,d=u.getTagTime(),e=0,h=!1;return{getTotalTime:function(){return e},stop:function(){c=0;d=u.getTagTime();h=!1},mark:function(){a=h?u.getTagTime()-d:0;c+=a;c>=g&&(b=c-a<g,e+=b?c:a);d=u.getTagTime();
h=!0}}},O=function(g,a){var b=g||l.adsafeSrc||l.requrl;b&&0===b.indexOf("http")||(b=0===b.indexOf("//")?l.protocol+":"+b:l.protocol+"://"+b);var b=/((http|https):\/\/(([^\/\.]*)\.([^\/]*)))(?:\/(.[^?]*)\??(.+)?)*/.exec(b),c=b[1],d=b[2],e=b[3],h=b[4],s=b[5],n=g?b[6]:"",q=a?b[7]:"",r={},z=function(a){f.isDef(a)&&(h=a);return h},k=function(a){f.isDef(a)&&(s=a);return s},w=function(a,b){return a+"="+f(b).toION()},q=q?q.replace(/&$/,""):"";return{appendToParamValue:function(b,c){var d,e;(d=r[b])?r[b]=
d+c:a&&q.length&&(d=q.split(b),e=1<d.length)&&(e=d[1].split("&")[0],d=[d[0],e].join(b),q=q.replace(d,d+c))},fullDom:c,hostname:e,sub:z,master:k,setParam:function(a,b){r[a]=r[a]||{};f(b).isObj()?f(r[a]).mixin(b):r[a]=b},path:function(a){n=a},toString:function(){var a=z()?z()+".":"",b=n?"/"+n:"",c;q||!f(r).isEmpty()?(c=f(r).isEmpty()?"":f(r).stringify(w,"&"),c="?"+q+(q&&c?"&"+c:c)):c="";return d+"://"+a+k()+b+c}}},Ub=function(){var g=function(a){var c=[];f(a).each(function(a,b){400>=c.concat(b).join("").length&&
c.push(b)});return c},a=function(a,c){var d;c&&c.id&&(d=c.id.replace(/[^\w-_.>\/]/g,""));return d};return function(){var b,c;try{if(u.friendly&&!u.isMobileApp())if(b=m.getAggregator().request("adSlotIds"))c=b;else{var d,e;if(d=t.getOurNodeInTop()){var h=t.getAncestorNodes(d);e=f(h).map(a)}if(b=e=e&&g(e))m.getAggregator().trigger("addOutputItem",{output:b},"slid",{type:k.DT_CODES.ADTALK,asION:!0}),m.getAggregator().provide("adSlotIds",b),c=b}return c}catch(s){m.getAggregator().trigger("error",k.ERROR_CODES.AD_SLOT_ID)}}()},
...
a.documentMode?a.documentMode:k.NA},getBrowserType:b,getUserAgent:c,params:function(){var c=b(),e;e=["{45EA75A0-A269-11D1-B5BF-0000F8051515}","{3AF36230-A269-11D1-B5BF-0000F8051515}","{89820200-ECBD-11CF-8B85-00AA005B4383}"];var h=k.NA,s=p.getDoc(),n=s.createElement("div");if(a(g.IE))try{if("-ms-ime-align"in s.documentElement.style)h="11";else for(n.style.behavior="url(#default#clientcaps)",s=0;s<e.length&&!(h=n.getComponentVersion(e[s],"componentid").replace(/,/g,"."));s++);}catch(q){}e=h;h=k.NA;
n=p.getWindow();f.isDef(n.navigator)&&f.isDef(n.navigator.appName)&&(h=n.navigator.appName.toLowerCase()[0]);return{br:c,abv:e,an:h}},isAndroidWebViewBrowser:function(){var a=c(),b=["SamsungBrowser","FB_IAB","Silk"],h=function(){return f(b).findFirst(function(b,c){return I.contains(a,c)})};return I.contains(a,"Android")&&h()},getIOSVersion:function(){var a,b;(a=c())&&(a=a.match(/OS\s\d+/))&&0<a.length&&(b=(b=a.toString().match(/\d+/))&&0<b.length&&parseInt(b.toString()));return b}}},hc=function(){var g=
function(){return!1},a=function(){return k.NA};return{getDocumentMode:a,getBrowserType:function(){return"u"},getIOSVersion:function(){},params:function(){return{br:"u",abv:a(),an:a()}},browserIs:g,getUserAgent:g,hasPostMessage:g,hasAncestorOrigins:g,hasIntersectionObserver:g,hasMutationObserver:g,isAndroidWebViewBrowser:g}},Ua=function(){return{createInstance:function(g,a){return u.isDomless()?new jc(a):new kc(g,a)}}},lc=function(){var g,a=function(a,c){var d="",e="",h=p.getDoc().getElementsByTagName("ins");
...
'"></iframe>'],img:['<img src="','"/>']}[a];d=d[0]+b+d[1];u.isOmid()?(e=new lc,e.isApplicable(b)?e.deployBlockingScriptIntoIframe(b):c(a,b,d)):"true"===l.forceAppend?c(a,b,d):K.write(d)},c=function(a,b,c){var d=!c&&y.getXHR2();d?(d.open("POST",a),d.onreadystatechange=function(){4===d.readyState&&200===d.status&&b&&b()},d.send()):(c=t.createImage(),b&&(c.onload=b),c.src=a)},d=function(a,b,c,d){var e=K.createElement("script");c=c||l.contextNode.parentNode;e.type="text/javascript";e.src=a;b&&(e.onload=
...
"ex",{type:k.IMPRESSION_EVENT}))}catch(q){a.add(k.ERROR_CODES.EXCHANGE_PARSING)}}}},mc=function(){return{parse:function(a){}}},kb=function(){var a={},b=function(b){var c;f.isUndef(a[b])&&(c=l.jsFeatures,a[b]=!1,-1!==c.indexOf(b)&&(c=RegExp(b+"(?=$|,)|"+b+":(?!,|$)(.?\\d*(?=,|$))").exec(c)))&&(c=c[1],a[b]=!c||c>100*p.random());return a[b]},c={es:"everySecond",sc:"usesca",ha:"usehaps"},d={fif:"useFIF",gmnp:"groupm_native_publisher","for":"forceAppend",b11:"b11",cnod:"cnod"};l.integration&&(d.intblk=
...
e,h,s){var f=c(),q,r,k;s=s||"ias_callback";d=""+d;RegExp(s).test(d)&&(q=RegExp("("+s+"=)(.[^&]*)").exec(d)[0],r=q.split("=")[1],k=I.stringToFn(r),d=a(d,q));x[f]=function(a){e(a);h&&k&&k(a);x[f]=void 0};return d=b(d,s,f)},wrapToGlobal:function(a){var b=c();x[b]=function(c){a(c);x[b]=void 0};return b}}},Ib=function(a,b,c){var d=function(a,b){var c=a,d;b&&(d=","+f(b).stringify(function(a,b){return a+":"+b},","),c=O(a,!0),c.appendToParamValue("adsafe_jsinfo",d),c=c.toString());return c},e={jss:{isFW:!0,
...
getDoc:function(){u.isDomless();return K},getIntersectionObserver:function(a,b){u.isDomless();return new IntersectionObserver(a,b)},getMutationObserver:function(a){u.isDomless();return new MutationObserver(a)},pow:function(a,b){return Math.pow(a,b)}},vb=function(){return{createInstance:function(){return u.isDomless()?new pc:new qc}}},qc=function(){var a,b=!1,c=!1,d=m.getAggregator().request("features"),e=new ca,h=function(){var b;d.on("blur")?(b=p.getWindow(),b=u.friendly&&b.top.document&&(f.isFunction(b.top.document.hasFocus)||
...
c=a.split(".");for(a=0;a<c.length&&(b=b[c[a]],!f.isUndef(b));a++);return b},contains:function(a,b){return f.isStr(a)?-1!==a.indexOf(b):!1},trim:function(a){return f.isFunction(a.trim)?a.trim():a.replace(" ","")}},tc=function(){var a=function(){var a=function(a){return a&&a.width&&30<a.width&&a.height&&30<a.height},b=function(b){b=t.getDimensionFromAttributes(b);return a(b)};return{hasValidDimensions:function(c){var d;(d=b(c))||(c=t.getDimensionFromStyle(c),d=a(c));return d},hasValidDimentionAttributes:b}}(),
...
b){if(b.addedNodes.length){var c=b.addedNodes[0];c&&t.isElement(c)&&t.tagNameIs(c,"IFRAME")&&f.debounce(q,10)}})})});b.observe(a.body,{attributes:!1,childList:!0,characterData:!1})};a&&a.body&&(p.execAtEndOfThread(q),!n&&f.isDef(x.MutationObserver)&&r())},zc={name:"BS_InApp",dependencies:[],creator:function(){var a=p.getWindow().document;m.getAggregator().request("mobileApp").isMobileAppEnvironment()&&Ea(a,"bs1",3,100)},settings:{tier:k.TIERS.VIEWABILITY,applies:function(a,b,c){b=c.isAvid()||c.isOmid()||
...
new Bc:new Cc(a)}}},Cc=function(a){var b,c,d,e=new xa,h=new da,s=new Dc(e),n=new Ec,q=new Fc,r=new Gc(e,a),k=function(){f.isUndef(d)&&(d=new Hc(e));return d},l=function(){e.provide({frameCollection:function(){return s},adProxy:function(){return q},adTalkMessage:function(a){return new Ic(a,e)},adTalkMessageCollection:function(){return n},idMapModule:k,interFrameQuerySelector:function(a){return new Jc(a)}})};return{isApplicable:function(a){return f.isDef(x.JSON)&&f.isDef(x.postMessage)&&!a.isAvid()&&
...
q+=c.isMe?"*":"";q+=f.length?"."+f[0].tagId:"";n+=q}return n}},d=function(a,b){var c="";return c=a.length?c+f(a).stringify(function(a,c){return b+(l.asid===c.adSafeId?"*":"")+"."+c.tagId},"|"):c+b};return{output:function(){var b=a.request("frameCollection"),d=f(b.list).stringify(c,"|");return f.toBase(b.getMe().unifiedId,62)+"+"+d}}},Hc=function(a){var b=a.request("frameCollection").getMe().getStringifiedPosition(),c=l.contextNode.parentNode,d=a.request("interFrameQuerySelector",c),e=function(a){return a&&
...
b){var c;a!==l.asid&&h(b)&&(c=b.positionStr+"."+b.srcAsid+"."+b.transferDuration+"_"+b.srcTagId);return c});c.push(d);c=c.join("|");120<c.length&&(c=e(c),m.getAggregator().trigger("stopAdTalk"));return c},isCandidateForMyIdMap:h}},Jc=function(a){var b=new da,c=function(a){var b=a,c="IFRAME"===t.getNodeName(a),d=t.isWindow(a);c?b=a.contentWindow.document:d&&(b=a.document);return b},d=function(a,b){return t.querySelector(c(a),b)},e=function(c,e){var k,q,r;try{q=d(c,e),q||(k=b.getFrames(!0,a),f(k).findFirst(function(a,
...
l),e()):u.addInformationFromSrc(d,l);(t=b&&a.request("idMapModule").isCandidateForMyIdMap(d))&&s()}catch(ue){m.getAggregator().trigger("error",k.ERROR_CODES.ADTALK_DUBIOUS)}};return{sendAdTalkCall:s,sendToKnownFrames:function(){(new ea).send(h)},startListening:function(){c=new ya;c.listen(e,n,h)},stopListening:function(){c&&c.stop()},validateMessage:e,processMessage:n,createMessage:h}},Lc=function(){var a=m.getAggregator().request("ids"),b=a.getAsid();return{tagId:a.getFwId(),adSafeId:b}},Fc=function(){var a,
...
isMeasurable:0===d||75<=d,isObstructed:!1,isHidden:!1,numberOfAdverts:1,percentageInView:d,detectionMethod:"av"},b.trigger("screenLocationChanged",k)}catch(q){}};return{start:function(){try{var b=a.getAvidAdSessionContext().avidAdSessionType;c="display"===b||"video"===b;a.addEventListener("viewability",d)}catch(h){}},addScreenLocationChangedListener:function(a){b.on("screenLocationChanged",a)}}},Zc=function(a,b){var c=[],d=(new Date).getTime(),e=0,h=0,s=!1,n=new Da,q=m.getAggregator().request("omidAdSessionContext"),
...
return{getOmidMeasurementStrategies:function(){var a=m.getAggregator().request("omidVerificationClient"),b=new aa(a),e=new Ba,h=[];if(b.accepts()&&!e.accepts()){var f=new ba,k=new bd,e=new cd(a,new D),f=[{environment:f,measurementStrategy:new dd(a,new D)},{environment:k,measurementStrategy:new Ha(new ia,p.getWindow(),new D)}],f=(f=(new ha(f)).resolve())?f.measurementStrategy:{},q=new Ia(new Ja(new T),new T,k.accepts()),k=new Vc(a,b,l.mode),r=new Ka(e,f,new D,q),b=new Wc(a,b,l.mode),a=new La(e,f,new Zc(a,
new D),new D,q);h.push({environment:k,measurementStrategy:r});h.push({environment:b,measurementStrategy:a})}return h},getAvidMeasurementStrategies:function(){var b=m.getAggregator().request("avidJsClient"),d=new Tc(b),e=[];if(d.accepts()){var h=new Sc(b,d,l.mode),f=new Uc(b,d,l.mode),k;k=new Xc(b,new D);if(d.supportsAdContainerGeometry()){var q=new ed,r=new fd,z=new Ha(new ia,p.getWindow(),new D),B=new gd(b,new D),q=(q=(new ha([{environment:q,measurementStrategy:B},{environment:r,measurementStrategy:z}])).resolve())?
q.measurementStrategy:{},r=new Ia(new Ja(new T),new T,r.accepts()),z=new hd(b,new D),b=new Ka(z,q,new D,r);k=new La(z,q,k,new D,r)}else r=new Yc(b,new D),b=new Ga(r,new D),k=new Ma(r,k,new D);e.push({environment:h,measurementStrategy:b});e.push({environment:f,measurementStrategy:k});a.setAvidAdSessionContext(d.getAvidAdSessionContext())}return e},getMraidMeasurementStrategies:function(){var a=[],d=b.isSafeToInjectMraid(),d=new id(p.getWindow(),d,b.isMobileAppEnvironment());if(d.accepts()){var e=m.getAggregator().request("context"),
e=new jd(p.getWindow(),e),h=new kd(p.getWindow(),new D);e.accepts()?(d=new ld(p.getWindow(),new D),d=new Ma(h,d,new D),d={environment:e,measurementStrategy:d}):(e=new Ga(h,new D),d={environment:d,measurementStrategy:e});a.push(d)}return a}}},ha=function(a){return{resolve:function(){var b=f(a).findFirst(function(a,b){return b.environment.accepts()});if(f.isDef(b))return b}}},ad=function(a){var b=!1,c,d={winDimensions:E({scrX:0,scrY:0,width:0,height:0}),adDimensions:E({scrX:0,scrY:0,width:0,height:0}),
...
q&&(v+=l-k,k=l);if(k==l||v===h-1)w.push(q),k=0}c=w}}else c=b;return m},getObstructionStatusBySlice:function(){return c}}},cd=function(a,b){var c=m.getAggregator().request("omidAdSessionContext"),d=null,e=!1,h=new ga,s=function(){var a=p.getDoc().getElementById("iasbi"+l.asid);a&&a.hasAttribute("CreativeLoaded")?(e=c.delayingViewabilityEvents=!1,d&&(n(d),d=null)):p.setTimeout(s,50)},n=function(a){if(a&&a.data&&a.data.adView){var f=a.data.adView,n=!!(f.measuringElement&&f.containerGeometry&&f.onScreenContainerGeometry);
...
ba=function(){var a=m.getAggregator().request("omidAdSessionContext");return{accepts:function(){return a&&(a.isNative||a.isLimitedSandbox)}}},dd=function(a,b){var c=m.getAggregator().request("omidAdSessionContext"),d=null,e=!1,f=new ga,s=function(){var a=p.getDoc().getElementById("iasbi"+l.asid);a&&a.hasAttribute("CreativeLoaded")?(e=c.delayingViewabilityEvents=!1,d&&(n(d),d=null)):p.setTimeout(s,50)},n=function(a){if(a&&a.data&&a.data.adView)if(c.delayingViewabilityEvents)d=a,e||(e=!0,s());else{var n=
...
a;b.start(!0);m.getAggregator().on("IOPivChange",e);m.getAggregator().on("nodesAdded",f)}}},zd=function(a,b,c,d,e,h){var s,n,q,r,z,B,w,v,N,M=t.getElementsWindow(b);t.getElementsDocument(b);m.getAggregator().provide("iframeCalculatorHelper",sd(M));var A=td(b,d),C=k.AD_IDENTIFIER+"-"+l.asid,D=!1,Q=E({}),F=u.isDeviceTypeGroupMobile(),G=u.usesGroupMCustomMetricMobilePassThru(),H,I=function(){if(!s)if(s=A.calcDims(b,d,q),c===k.DETECTION_METHODS.VIDEO&&u.embedded){var a=s,e=(new E(b)).getRounded();try{a.set("width",
...
k.applies(n,l,h)?(q=Mc(b,s),q.start(),D=!0,G&&(c=new ud(I().getRounded()),k=new vd,k=new wd(q.getObservedNode(),k),r=new xd(c.getSlices(),k))):G&&(z=new yd(a))})();return{mark:function(){t.setAttributeOf(H,C,"")},cleanup:function(){t.removeAttributeOf(H,C)},isNode:function(a){return b===a},isHidden:U,isObstructed:function(){return!1},getDims:I,getPercentInView:L,getLocationState:S,getOnPageViewability:function(){return U()?k.OUT_OF_VIEW:S()},getDetectionMethod:function(){return c},getContainerDims:function(){return A.allowsOptimization?
...
allowForAdContainerChange:!0}},Ld=function(){var a,b=p.getDoc().scripts,c=function(){a||(a=f(b).findFirst(function(a,b){return I.contains(b.src,"servedby.flashtalking")}));return a},d=function(){var b,d=a||c(),f=/;(\d+);/;d&&d.src&&(b=d.src.match(f)[1]);return b};return{detectionMethod:k.DETECTION_METHODS.BUSTED_IFRAME_FLASHTALKING,isApplicable:function(a,b){return b.on("bustediframe")&&a.embedded&&void 0!==c()},getCssSelector:function(){var a,b=d();try{a=f(["#ftdiv"+b,"#ftframe"+b]).findFirst(function(a,
b){return t.crossQuerySelector(b,!0)})}catch(c){}return a}}},Md=function(){return{isApplicable:function(a,b){var c;try{c=t.getFrameId()}catch(d){}return b.on("yieldmo")&&c&&I.contains(c,"_tpi")&&t.crossQuerySelector("[data-href*='ads.yieldmo.com']",!0)},getCssSelector:function(){return"[id='"+t.getFrameId().replace("_tpi","")+"']"},detectionMethod:k.DETECTION_METHODS.BUSTED_IFRAME_YIELDMO}},Nd=function(){var a,b,c=p.getDoc(),d=function(){var b;a||(a=(b=(b=t.querySelector(c,"div[id*='ebDiv']"))&&b.id)&&
...
!f(c.EBG.ads).isEmpty();return b.on("sizmek")&&m&&c&&k&&d()&&e()},getCssSelector:e}},fa=function(){var a,b=m.getAggregator().request("features");a||(a=[Nd(),Md(),Ld()]);return f(a).findFirst(function(a,d){return d.isApplicable(u,b)})},Gd=function(){var a,b=function(){p.execAtEndOfThread(function(){m.getAggregator().trigger("delayedViewabilityReadyCallOnly")})};return{usePlaceholder:!1,detectionMethod:k.DETECTION_METHODS.VENDOR_CELTRA,isApplicable:function(a,b){return b.on("celtra")&&a.isCeltra()},
...
return{usePlaceholder:!0,detectionMethod:k.DETECTION_METHODS.AD_HUNT,isApplicable:a.isMobileAppEnvironment,find:function(){var a=t.findChildWithLargestContent(p.getDoc().body);return a&&a.hasValidSizeForMobileApp&&!a[k.IAS_DETECTOR]?a:void 0},getAdContainer:function(){return l.contextNode.parentNode},allowForAdContainerChange:!0}},Dd=function(){var a;return{detectionMethod:k.DETECTION_METHODS.SPECIFIED_AD,isApplicable:function(){return u.isSpecifiedAd()},find:function(){var b=t.crossQuerySelector(l._cl_adpath);
b&&(a=t.getParent(b),1>t.getNodeArea(b)&&(a=b,b=t.findElementsWithSize(b)));return b},getAdContainer:function(){return a},allowForAdContainerChange:!0}},Cd=function(){var a;return{detectionMethod:k.DETECTION_METHODS.SPECIFIED_AD_COMPOUND,isApplicable:function(){return u.isCompoundAd()},find:function(){var b=[],c;/\[(.*)\]/.exec(l._cl_adpath)[1].split(",").forEach(function(d){if(c=t.crossQuerySelector(d))a=t.getParent(c),1>t.getNodeArea(c)&&(a=c,c=t.findElementsWithSize(c));b.push(c)});return b},getAdContainer:function(){return a},
...
a=!t.elementIsEmbedded(a);return b||a}}},Pd=function(){var a=m.getAggregator().request("iframeCalculatorHelper");return{calcDims:a.calcDims,getClippedDims:a.getClippedDims,allowsOptimization:!0,isApplicable:function(a){return!(u.isSpecifiedAd()||u.isBustedIframe())&&t.elementIsEmbedded(a)}}},Od=function(){var a=m.getAggregator().request("iframeCalculatorHelper");return{calcDims:function(b){var c=a.calcDims();b=(new E(b)).getRounded();c.set("height",b.height);c.set("width",b.width);return c},getClippedDims:a.getClippedDims,
allowsOptimization:!1,isApplicable:function(a,c){var d=(u.isSpecifiedAd()||u.isBustedIframe())&&u.embedded&&!t.nodeIsInWindow(c,p.getTop()),e=u.isCeltra()&&t.elementIsEmbedded(a);return d||e}}},Rd=function(){var a;return{calcDims:function(b,c,d){d=f.isDef(d)?d.getAdNodeDimensions():void 0;a&&f.isUndef(d)&&(d=a.calcDims(b,c));return d},isApplicable:function(a){a=Y();var c=m.getAggregator(),d=c.request("browser"),e=c.request("features"),c=c.request("context");return c.isDeviceTypeGroupMobile()&&a.applies(d,
...
b.addedNodes.length&&(c=!0)});c&&(m.getAggregator().trigger("nodesAdded"),b.disconnect())};Bd(a);return{find:function(){s.isUsingPlaceholder()||!s.hasAd()?n():s.refresh();return s},getDetectionMethod:function(){}}},Sd=function(){var a=function(){return null};return{find:a,getDetectionMethod:a}},mb=function(a){var b,c=[],d=a||x,e=function(){return"stub"===k().mode};a=function(){e()&&(b=d.setInterval(function(){if("stub"!==k().mode){d.clearInterval(b);for(var a=0;a<c.length;a++)d.avid.addEventListener(c[a].type,
...
Na=function(){var a=function(a,c){var d,e=Wd[a];e.detect(c)&&(d=e.STRINGIFIED_METHOD);return d};return{process:function(b){return f(b).map(a).join("")}}},Xd=[{STRINGIFIED_APPDET:"omid",detectionRules:{javascript:["omidSupported"]}},{STRINGIFIED_APPDET:"avid",detectionRules:{javascript:["avid.getAvidAdSessionContext"]}},{STRINGIFIED_APPDET:"file",detectionRules:{url:["file:"],scriptSrc:["file:"]}},{STRINGIFIED_APPDET:"cont",detectionRules:{url:["content:"]}},{STRINGIFIED_APPDET:"appl",detectionRules:{url:["applewebdata:"]}},
...
b.slice(1);b in a&&(c=a[b]);return c}}},Oa=function(){return{getVolumeFromContext:function(a){var b,c=-1;f.isDef(a)&&(f.isDef(a.volume)?c=a.volume:f.isDef(a.adVolume)&&(c=a.adVolume));-1!==c&&(b=c);return b}}},gb=function(a,b,c){var d="IASid"+a,e="getIasVidBridgeVersion"+a,h,s,n,q=[],r=k.OUT_OF_VIEW,t=!1,x=!1,w=!1,v=!0,A=new ae,C=be(),D=new Zd,E=new Oa,G=new Da,I=function(b){var d;t=!0;var e={};try{e=JSON.parse(unescape(b&&b.data?b.data:b))}catch(f){e={}}e.id==a&&(d=A.registerEvent(e,c.getCurrentLoc()),
...
"message",function(a){I(a)});m.getAggregator().on({volumeChanged:L});m.getAggregator().provide({videoEventsString:A,videoVersion:function(){var a,b=H();try{a=b.getVersion()}catch(c){a=-1,m.getAggregator().trigger("error",k.ERROR_CODES.NO_VIDEO_AD_INTERFACE)}return a},videoTranslator:function(){return s=s||new ce},adNode:H});H();D.listenForResult();C.isApplicable(u)&&C.start(c)})();return{findAdNode:H,getViewStateOverride:function(){return r},isHandshakeComplete:function(){return t},triggerInitializationEvents:O,
setVideoSpecificScreenEventFields:function(a){var b=a.viewState===k.NA;a.isVideoPlaying=!0;a.isSoundOn=v;a.isVideoPlayingInFullscreen=!1;if(r){if(r===k.OUT_OF_VIEW){var c=a.reason?a.reason.split("."):[];c[c.length]=k.OUT_OF_VIEW_REASONS.VIDEO;a.reason=c.join(".")}a.viewState=b?a.viewState:r;a.isVideoPlaying=r===k.IN_VIEW}w&&x&&(a.percentInView=100,a.isVideoPlayingInFullscreen=!0)}}},de=function(){return new $(k.PERF_TIMELINE_TRACKER_CONFIG.WRAPPERS.AS.TYPE,k.PERF_TIMELINE_TRACKER_CONFIG.WRAPPERS.AS.COUNT)},
...
(m.getAggregator().trigger("addOutputItem",{output:h},d,k),c[d]={});a=!1})};return{start:function(){m.getAggregator().on("preSendDt",function(a){q(a);b&&b.kill()});m.getAggregator().on("addThrottledProp",l);m.getAggregator().on("addThrottledOutputItem",n)},addFieldTypes:function(a){f(a).each(function(a,b){c[b]={}})}}},Vb=function(){var a={},b=function(b){a[b]||(a[b]=new za(1,l.birthdate));return a[b]},c=function(a){b(a).start()},d=function(a){var c=b(a);c.stop();m.getAggregator().trigger("addThrottledProp",
"im",a,c.getTime())};return{start:function(){m.getAggregator().on({markTime:c,measureTime:d})},isApplicable:function(a){return a.on("pIntervals")}}},ee=function(){return new $(k.PERF_TIMELINE_TRACKER_CONFIG.WRAPPERS.JS.TYPE,k.PERF_TIMELINE_TRACKER_CONFIG.WRAPPERS.JS.COUNT)},Bb=function(a){var b={},c=a instanceof Array&&0<a.length;c&&(f(a).each(function(a,c){b[c.getType()]=c}),m.getAggregator().on("perfCheckpoint",function(a){var c=b[a.type];if(c)try{c.addPoint(a),c.isFinished()&&m.getAggregator().trigger("addThrottledProp",
...
l.asid),F=p.length,G,H,I=n(l.sp_imp_jsInfo_minLength,0),D="";try{G=t-F-I,0<G&&(H=ke(q,G,14,b.on("partialUrls")),D+=H.join("&"))}catch(K){a.add(k.ERROR_CODES.IMPRESSION_URLS)}p=y+D;q=p+="&"===p.slice(-1)?"":"&";var L,F="";try{L=t-p.length,F+=le(L,"adsafe_jsinfo=",a,b,c,e,f,s)}catch(O){a.add(k.ERROR_CODES.JSINFO),F+="adsafe_jsinfo=e:"+a.toString()}p=q+F;d.macroUrl&&(p+="&"===p.slice(-1)?"":"&",p+=d.macroUrl);r=p;d.sendImpression(r);l.impUrl=r;m.getAggregator().trigger("impressionsent")}catch(P){__IntegralASDiagnosticCall("impsend",
...
l.perf.mark("si");c.on("swapids")?n.push("oid:"+l.oid):n.push("uid:"+d.unq);n.push("v:19.8.121");n.push("sp:"+(l.isSplitMode?1:0));n.push("fwm:"+("true"===l.fwMonitoring?1:0));c.on("resolution")&&(n.push("wr:"+f(t.windowSize()).toArray().join(".")),n.push("sr:"+f(t.screenSize()).toArray().join(".")));b=function(){var a;a=String(l.scriptUrl).toLowerCase();var b=a.indexOf("ias_xpc");-1<b?(a=a.substring(b+7+1),b=a.indexOf("&"),a=-1<b?a.substring(0,b):a):a="NotFound";return a||""}();"NotFound"!==b&&n.push("x_xpc:"+
b);return n},ke=function(a,b,c,d){var e,h=[],k=function(a){return-1!==a.key.indexOf("q")||-1!==a.key.indexOf("g")};a&&a.length&&(e=f(a).map(function(a,b){if(k(b))return b}),a=f(a).map(function(a,b){if(!k(b))return b}),f(e.concat(a)).each(function(a,e){var f;f=e.val;var k=e.key,l;l=24+h.join("&").length+k.length+c;l=b-l;d&&l<f.length&&0<l&&(f=f.substr(0,l));if(f.length||""===e.val||""===e.type)f="adsafe_url="+f+"&adsafe_type="+k,k=h.join("&").length,f.length+k<=b&&h.push(f)}));return h},Mb=function(){return{isApplicable:function(a){a=
p.getWindow();return!(!a.navigator||!a.navigator.plugins)},start:function(a){a=p.getWindow();a=ne(a.navigator.plugins);var b=a.hash(),c=b.join(".");m.getAggregator().trigger("addOutputItem",{output:c},"pl",{type:k.IMPRESSION_EVENT});b.length&&(a=a.decode(b),a=a.join("."),m.getAggregator().trigger("addOutputItem",{output:a},"pd",{type:k.DT_CODES.ADTALK}))}}},qe=function(a,b){return{start:function(){try{var c=oe(),d=c.getMethodName(a),e=new O(b.getEnabledScriptUrl(a)),f=pe,l=p.getDoc();(new f(l.body||
l.head||l.documentElement,e)).getWindow()[d]=c.getMethod(b)}catch(n){m.getAggregator().trigger("error",k.ERROR_CODES.FEATURE_SCRIPT)}}}},oe=function(){var a=function(a,c,d){a===l.asid&&(d.unshift(c),m.getAggregator().trigger.apply(null,d))};return{getMethod:function(b){return function(c){var d={asid:l.asid,constant:k,features:b,context:u,dtBaseURL:l.dtBaseURL,spg:Qa};"function"===typeof c&&c(a,d)}},getMethodName:function(a){return"register_"+a}}},Tb=function(){return{start:function(a,b){f(a).each(function(a,
d){b.getEnabledScriptUrl(d)&&qe(d,b).start()})}}},ne=function(a){return{hash:function(){for(var b=0,c=a.length,d=[],e;b<c;b++)e=a[b].filename.replace(/\.plugin$/,""),e=I.hashCode(e),e=f.toBase(e,62).slice(-4),d.push(e);return d},decode:function(b){var c,d;c=p.floor(p.random()*b.length);d=encodeURIComponent(a[c].filename.replace(/\.plugin$/,""));return[b[c],d]}}},pe=function(a,b){var c,d=/MSIE [1-9][^0-9]/g;(function(b){var f;c=t.createHiddenIframe();c.setAttribute("src","about:blank");(a||l.contextNode.parentNode).appendChild(c);
