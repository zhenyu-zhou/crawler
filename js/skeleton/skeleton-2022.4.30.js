...
try {
	...
	var __IntegralASConfig = {
		...
		sp_cdnScripts: {xsca: "static.adsafeprotected.com/xsca.17.4.85.js", sca: "static.adsafeprotected.com/sca.17.5.12.js", main: "static.adsafeprotected.com/main.19.8.306.js"},
		protocol: 'https',
		jsref: "",
		asid: "e765d0f0-c89e-11ec-8753-1625d74abcc5",
		...
		accountForSadImps: '',
		fwMonitoring: '',
		mn: "jsserver-experiment-primary-7f5b6bbdc8-zj7km",
		mobOrTab: false,
		app: true,
		mobAppWebview: false,
...
n.getAggregator().trigger("addOutputItem",{output:k.mainBirthdate},"mtim",{type:l.IMPRESSION_EVENT});var z;try{b.isApplicable(v)&&(b.start(),c.addItem(b.getFrameMap(),"fm"),m.on("fm2")&&c.addItem(b.getFrameMapIncludingPeerCase(),"fm2"),m.on("idMap")&&(z=b.getIdMap())&&c.addItem(z,"idMap"))}catch(L){d.add(l.ERROR_CODES.ADTALK_GENERAL)}try{e.isApplicable(f,m,q)&&e.start(),C=p.detectTopURL(),m.on("exch")&&(w=(new jb).createInstance(d,c),w.parse(C)),x=kb(),x.isApplicable(m)&&x.start(m),q.usesGroupMCustomMetric()&&
lb(),t.start()}catch(L){d.add(l.ERROR_CODES.IMPRESSION_LEADUP)}k.perf.markEnd(sa);h=C;b=a.mVideo;try{mb(a.mErrors,a.mFeatures,a.mIds,a.mMode,a.iOutput,a.mBrowser,a.mViewability).send(h),g.isFunction(b.triggerInitializationEvents)&&b.triggerInitializationEvents()}catch(L){__IntegralASDiagnosticCall("impsend",L,k)}h=a.mBrowser;b=a.mIds;c=a.mViewability;d=a.mFeatures;e=a.mComm;f=a.mDataTransfer;p=a.mErrors;m=a.iOutput;q=a.loopDelay;t=a.mAdTalk;w=a.viewabilityMeasurement;a=a.mAncestorOrigins;x=v;try{k.perf.markStart(ta);
var F=nb();F.start();F.addFieldTypes([l.DT_SLOT.IM,l.DT_SLOT.FF,l.DT_SLOT.ENVIRONMENT,l.DT_SLOT.FF_EXP]);var B=ob(h,b,c);B.isApplicable(d)&&B.start();var M;d.bootstrapOn("getTpl")&&d.on("usetpl")&&(M=pb(e,f,p,b,m),M.init());qb(m,q,t,p,d,e,f,b,a);w&&w.start&&w.start();x.isDomless()||rb().start(["sca","xsca","rsrch"],d);sb();n.runTier(l.TIERS.VIEWABILITY);var I=tb();I.isApplicable(d)&&(I.start(),V.recordBlockingTime(),k.perf.mark(ub),V.setupOnLoadTracking(),V.setupBrowserDelayTracking());k.perf.markEnd(ta)}catch(L){p.add(l.ERROR_CODES.POST_IMPRESSION)}k.perf.markEnd(ra)}}
function vb(h,a,b,c,d,e,f,p,m,q){try{k.perf.markStart(ua),k.tpiLookupURL&&wb(f,p).init(k.tpiLookupURL),v.isDomless()||q.isStarted()&&q.sendOriginList(),n.runAll(),r.execAtEndOfThread(function(){var e;try{k.perf.markStart(va);e={output:(new Date).getTime()-d};h.addItem(e,"sinceFw",{type:l.DT_CODES.ADTALK});g.isDef(a.loopStarted())&&h.addItem({output:a.loopStarted()},"readyFired",{type:l.DT_CODES.ADTALK});k.perf.markEnd(va);var f,p;k.isSplitMode&&(p=k.protocol+"://"+k.sp_cdnScripts.main,k.perf.markResource(wa,
p));k.impUrl&&k.perf.markResource(xb,k.impUrl);f=k.perf.getData();n.getAggregator().trigger("addThrottledProp",l.DT_SLOT.IM,"prf",g(f).toION());b.isApplicable(v)&&b.sendAdTalkCall()}catch(C){c.add(l.ERROR_CODES.ADTALK_DELAY)}}),k.perf.markEnd(ua)}catch(t){c.add(l.ERROR_CODES.EVENT_LOOP_ONE)}}function yb(h){var a=new zb(h,v),b=new Ab,c=new Bb(h,v),d=Cb(h,v),e=(new Db(k,v,h)).resolve(),f=[a],p=[];d.applies()&&f.push(d);c.applies()&&f.push(c);b.applies(h,v)&&f.push(b);g(e).each(function(a,b){f.push(new Eb(b,
v));p.push(b.id)});0<p.length&&(n.getAggregator().trigger("addOutputItem",{output:p.join(".")},"scm",{type:l.IMPRESSION_EVENT}),n.getAggregator().trigger("addOutputItem",{output:p},"metricIdList",{type:l.DT_CODES.UNLOAD,asION:!0}));return f}function Fb(h,a){return!v.isVideo()||v.isAvid()||v.isOmid()||v.isInMobiMraidVideo()?{}:Gb(k.videoId,h,a)}function ib(){var h,a,b,c,d,e,f,p,m={};k.perf.markStart(xa);var q=n.getAggregator();Hb();m.mFeatures=q.request("features");m.mFeatures||(m.mFeatures=q.provide("features",
ya()));k.integration&&m.mFeatures.setReportedBootstrapFeatures("intblk","integration");k.autotagSizeSet&&m.mFeatures.setReportedBootstrapFeatures("ats","autotagSizeSet");k.encodeInvalidUrlChars=v.isDomless()?!1:m.mFeatures.on("encodeInvalidUrlChars");q.provide("mobileApp",function(){return h=h||new Ib(m.mFeatures)});q.provide("avidJsClient",function(){return a=a||new Jb(r.getWindow())});q.provide("context",v);m.mBrowser=q.provide("browser",(new Kb).createInstance());m.mErrors=q.request("mErrors");
m.mIds=q.provide("ids",Lb());m.iOutput=Mb(q);q.request("mJsonp");m.mComm=q.request("mComm");m.viewabilityMeasurement=(new Nb(Ob(m.mIds),m.mFeatures,m.mBrowser)).create();c=q.request("omidAdSessionContext");Pb(c,q);m.mFeatures.on("swapids")&&(k.oid=k.asid,k.asid=m.mIds.unq);m.mAdTalk=(new Qb).createInstance(m.mFeatures);b=Rb();c=Sb(q);d=q.provide("mPage",(new Tb).createInstance());m.mAncestorOrigins=Ub();m.mPageUrls=q.provide("pageUrls",(new Vb).createInstance(m.mAncestorOrigins,r));e=q.provide("mScreenEvents",
Wb());q.provide("ieXDomainViewability",Xb(m.mBrowser));f=Yb(m.mBrowser);p=Fb(m.mFeatures,e);m.mVideo=q.provide("video",p);Zb($b.build());f=(new ac).createInstance(m.mErrors,f,m.mFeatures);d=(new bc).createInstance(f,m.mErrors,d,v);p=q.provide("jobFactory",cc());m.mDataTransfer=dc(f,m.mErrors,m.mFeatures,m.mIds,m.iOutput,m.mBrowser,e,b);b=ec(m.mErrors,m.mFeatures,q,c);m.mMode=fc(b,m.mFeatures,m.viewabilityMeasurement.isImmediate());m.mViewability=q.provide("viewability",gc(D(),m.viewabilityMeasurement,
m.mBrowser,m.mDataTransfer,m.mFeatures,q,m.iOutput,d,e,m.mVideo));hc(m.mFeatures,m.mViewability,c,p);m.loopDelay=za();m.renderDetector=ic();m.mDataTransfer.setViewabilityMod(m.mViewability);k.perf.markEnd(xa);return m}function Aa(h){var a=h&&h.sca,b=h&&h.xsca,c=h&&h.w,d=h&&h.h;return function(e){e="string"===typeof e?JSON.parse(e):e;if(Object.assign)Object.assign(k,e);else for(var f in e)k[f]=e[f];k.isResolved=!0;k.sp_cdnScripts={sca:a,xsca:b};c&&d&&(k.adWidth=c,k.adHeight=d,k.autotagSizeSet=!0);
qa(k.isNewBuild)}}function Hb(){var h,a,b;b=n.getAggregator().request("omidAdSessionContext");h=b.app&&b.isDisplay&&!b.isWeb;(a=k.mobFwUrl&&-1<k.mobFwUrl.indexOf("/blocking/"))&&b.isLimitedSandbox&&"1"===k.adWidth&&"1"===k.adHeight&&(b.isLimitedSandbox=!1);h&&a&&(b.delayingViewabilityEvents=!0);if("app"===b.environment||!0===k.mobAppWebview)k.mobOrTab=!0}function Pb(h,a){if(h&&h.queuedOutputItems)for(var b=0;b<h.queuedOutputItems.length;b++)a.trigger("addOutputItem",{output:h.queuedOutputItems[b].value},
h.queuedOutputItems[b].code,{type:"impression"})}function hc(h,a,b,c){var d=yb(h);n.getAggregator().provide("viewabilityDefinitions",d);return g(d).map(jc(h,a,b,c))}function qb(h,a,b,c,d,e,f,p,m){var q;"true"!==k.minimizeCalls&&(q=(new Date).getTime(),r.execAtEndOfThread(function(){vb(h,a,b,c,q,d,e,f,p,m)}))}function Ba(h){if(k.isResolved)qa(k.isNewBuild);else{if("string"===typeof h)try{h=JSON.parse(h)}catch(e){}var a,b=h;a={37103:"https://vfw.amazon-adsystem.com/ias/j"};var c=/iasProxyPartnerId=([\d]+)/g.exec(b&&
b.config),c=c&&c[1];a=b&&b.host||a[c]||"https://jsconfig.adsafeprotected.com";c=h&&h.config;b=h&&h.contentType;try{var d=k.contextNode.src;-1!==d.indexOf("fw.js")&&-1!==d.indexOf("?")&&(k.staticFwjsMacros=d.split("?")[1])}catch(e){}c&&(d=a+"/jsconfig"+c+(k.adSessionId?"&adSessionId="+k.adSessionId:""),"application/json"===b?n.getAggregator().trigger("getJSON",d,Aa(h)):n.getAggregator().trigger("jsonp",d,Aa(h),!1,"cbName"))}}function jc(h,a,b,c){return function(d,e){var f,p,m;f=Ca(e,a);var q=kc(b,
h,e.rts).getCallbacks(),t=Da(c.createPingJobs(e.type,e.timeInViewThresholds,q,e.metricId));e.sendOtherwiseInViewSignal&&(m=Ca(e,a,e.sendOtherwiseInViewSignal),p=Da(c.createPingJobs(e.type,e.timeInViewThresholds,q,e.metricId,e.sendOtherwiseInViewSignal)));e.rtsCallbacks=q;e.thresholdType===l.GROUPM_MOBILE_PASS_THRU_TIME_IN_VIEW_THRESHOLD_TYPE?f=lc(f,e,t,v.isVideo()):(f=Ea(f,Z(e.minUnit),t),e.sendOtherwiseInViewSignal&&(p=Ea(m,Z(e.minUnit),p,e.sendOtherwiseInViewSignal),p.start()));f.start();return f}}
function mc(h){var a,b,c;k.mainBirthdate=k.hasOwnProperty("birthdate")?(new Date).getTime()-k.birthdate:-1;k.perf.markStart(Fa);k.isNewBuild=h;h=[nc,oc,pc,qc,rc,sc,tc,uc,vc,wc,xc,yc,zc,Ac,Bc];for(a=0;a<h.length;a+=1)n(h[a].name,h[a].dependencies,h[a].creator,h[a].settings);n.startSystem();h=n.getAggregator();h.provide({omidAdSessionContext:{}});h.provide({omidAdSessionVerificationParameters:{}});k.isResolved&&h.provide("features",ya());a=Cc();b=Ga();c=(new Dc).createInstance(a,b);h.provide({mErrors:a,
mJsonp:b,mComm:c});a=__IASOmidVerificationClient;h.provide({omidVerificationClient:a});h=[{environment:da(a),adSessionReadyStrategy:Ec(a)},{environment:Fc(),adSessionReadyStrategy:Gc()}];(new Hc(h)).resolve().adSessionReadyStrategy.start();k.perf.markEnd(Fa)}var Ha={};if(Object.assign)k=Object.assign({},k);else{for(var Ia in k)Ha[Ia]=k[Ia];k=Ha}var g=function(h){return new G(h)},G=function(h){this.iterable=h};G.prototype.isObj=function(h){var a=null===this.iterable,a="object"===typeof this.iterable&&
...
xa="in",sa="pr",ta="po",ub="bl",ua="lo",va="lt",wa="md",xb="id",hb=function(h){return{verifyContextNode:function(){if(!(__IASScope.isDomless||h.contextNode&&null!==h.contextNode.parentNode)){var a,b=J.getElementsByTagName("script"),c=b.length,d=b[c-1],e=h.scriptUrl;try{for(;0<=--c;)if(a=b[c],a.src&&0===a.src.indexOf(e)&&null===a.getAttribute("data-ias-script-tag")){d=a;a.setAttribute("data-ias-script-tag","found");break}}catch(f){}h.contextNode=d}}}},Jc=function(){var h,a=["STYLE","SCRIPT","HEAD",
...
l.AD_IDENTIFIER+"-"+k.asid+"]"),m=g.isDef(x)?x:m);x=m=g.isDef(m)?m.parentNode||m:m;c++}m=x}else m=p;if(u.isWindow(m)){try{w=m.document}catch(F){w=k.contextNode.ownerDocument}p=w}else"IFRAME"===u.getNodeName(m)?(f.push(p),p=p.contentWindow.document):p=m;var w=[],t=(m=p&&p.getElementsByTagName&&p.getElementsByTagName("*"))&&m.length,C;if(m)for(var z=0;z<t;z++)C=(x=m[z])&&x.nodeName&&x.nodeName.toUpperCase(),!x.children||0!==x.children.length||g(b).contains(C)||g(a).contains(C)||g(w).contains(C)||w.push(C);
w=b.concat(w);(new Ic(w,f)).searchDescendants(p)}};n.getAggregator().on("creativeFinderBatchCompleted",p);return{find:p,getClosestContainerName:function(){return u.getNodeName(h)}}},Ic=function(h,a){var b=function(b){function c(b){var c=u.getNodeName(b),d=u.isNodeCreative(b)||"IFRAME"===c;c&&d&&!u.isEmptyFriendlyFrame(b)&&(c=u.getNodeArea(b),c>m&&!g(a).contains(b)&&(p=b,m=c))}function e(){do c(b[f]),f+=1;while(0!==f%500&&f<b.length);f<b.length?r.execAtEndOfThread(e,0):n.getAggregator().trigger("creativeFinderBatchCompleted",
p)}var f=0,p=null,m=-1;if(b.length&&n.getAggregator().request("useYieldSearch"))e();else{for(f;f<b.length;f++)c(b[f]);n.getAggregator().trigger("creativeFinderBatchCompleted",p)}};return{searchDescendants:function(a){a=a||k.contextNode.parentNode;var c,e,f=[];for(c=0;c<h.length;c++){e=h[c].toLowerCase();var p=a.getElementsByTagName&&a.getElementsByTagName(e);if(p&&p.length)for(e=0;e<p.length;e++)f.push(p[e])}b(f)}}},ic=function(){var h,a=!1,b=!1,c=!1,d={status:l.RENDER.STATUS.OTHER,details:l.RENDER.DETAILS.OTHER},
...
(A.addEvent(a,"ready",c),h.save(l.RENDER.DIAGNOSTIC.MRAID_UNKNOWN));z(d)},C=function(a){a===l.RENDER.STATUS.DETECTED&&n.getAggregator().trigger("perfCheckpoint",{type:l.PERF_TIMELINE_TRACKER_CONFIG.RENDER.TYPE,timestamp:r.now(),code:l.PERF_TIMELINE_TRACKER_CONFIG.RENDER.CODE})},z=function(b){var c=h.build();b=b.status||d.status;c=c?c:d.details;a&&b===l.RENDER.STATUS.DETECTED?(C(b),n.getAggregator().trigger("addThrottledOutputItem","rend",b),n.getAggregator().trigger("addThrottledOutputItem","renddet",
c)):(C(b),n.getAggregator().trigger("addOutputItem",{output:b},"rend"),n.getAggregator().trigger("addOutputItem",{output:c},"renddet"))},k=function(){n.getAggregator().on("primaryadfound",function(a){var b,c=d,e=new Jc,f=!0;n.getAggregator().on("evaluateCreativeFinderResult",function(a){if(u.isNodeXDomainIframe(a))b?r.clearInterval(b):f=!1,c=w(a);else if(a){var d=!1,p=u.getRect(a),q;if(q=a&&"IMG"===u.getNodeName(a)){q=!0;var x=g.isDef(a.naturalWidth)?a.naturalHeight*a.naturalWidth:0,k=getComputedStyle(a),
C=0===a.src.length&&K.contains(k.content,"url"),n=a.getAttribute&&a.getAttribute("data-ias-rend-bi");0<x&&!C&&(q=!1);"false"===n&&(h.save(l.RENDER.DIAGNOSTIC.CONTENT_STYLED),q=!1);C&&!n&&t(a,k)}q&&(d=!0,h.save(l.RENDER.DIAGNOSTIC.BROKEN_IMAGE));!d&&p.width>=l.RENDER.WIDTH_THRESHOLD&&p.height>=l.RENDER.HEIGHT_THRESHOLD&&(m(c),b?r.clearInterval(b):f=!1);h.saveNode(a)}else a=e.getClosestContainerName()||h.getAdNodeName(a)||c.details,h.save(a);z(c)});var p=function(){var b=a&&a.getAdNode();try{e.find(b)}catch(N){n.getAggregator().trigger("renderdiag",
...
b){return d(function(c,d){var e=d.props,f;a(e)&&(f=d.output(),e.encode&&(c=encodeURIComponent(c),f=encodeURIComponent(f)),g.isFunction(b)?b(c,f):b[c]=f)})},iterate:d,cleanup:function(){d(function(a,c){c.props.flagForRemoval&&delete b[a]})},getItem:function(a){return b[a]}}},Oc=function(){var h={},a=function(a,c){h[a]=c};return{request:function(a){var b,d=h[a],e=g(arguments).toArray();e.shift();g.isUndef(d)||(b=g.isFunction(d)?d.apply({},e):d);return b},provide:function(b,c){c?h[b]=c:g(b).each(a);
return c}}},bc=function(){return{createInstance:function(h,a,b,c){return v.isDomless()?new Qc:new Rc(h,a,b,c)}}},Rc=function(h,a,b,c){var d;return{collect:function(){var a=h.find(),f=a.getOutOfViewReasons(),p=b.isHidden(),m=c.isDeviceTypeGroupMobile()&&d?d:E(u.calcWinDims()),q=a.getDims();d=m;if(!a.hasAd()||!m.hasValidDims()||!q.hasValidDims())return{viewState:l.NA,posViewState:l.NA,embedded:v.embedded,winDimensions:m,adDimensions:q};!0===p&&f.push(l.OUT_OF_VIEW_REASONS.FOCUS);return{winDimensions:m,
...
Qc=function(){return{collect:function(){return{}}}},Sc=function(h){h=h||g.identity;var a=[],b=r.now(),c=function(){var c=r.now();a.length&&(a[a.length-1].duration+=c-b,b=c)};return{clear:function(){c();a=[]},fastForward:c,get:function(){return a},hasAlwaysBeen:function(b){return 1===a.length&&a[0].state===b},addState:function(b){var d=a[a.length-1];b=h(b);c();0!==a.length&&b===d.state||a.push({state:b,duration:0})}}},Ma=function(h,a){var b=h||1,c=0,d=0,e,f=0;return{start:function(){0===c%b&&(e=r.now())},
stop:function(){var p=e||a;0===c%b&&(f+=r.now()-p,d++);c++},getTime:function(){return f},getCount:function(){return d}}},Na=function(){var h,a=0,b=0,c=new D(!0),d=function(){h&&(a++,c.trigger(a),b>a?r.execAtEndOfThread(d):e())},e=function(){h=!1;a=0};return{onTick:function(a,d){var e=g.collapseArgsIntoHash(a,d);g(e).each(function(a,c){a=parseInt(a);b=a>b?a:b});c.on(e)},start:function(){h||(h=!0,r.execAtEndOfThread(d))},kill:e,isActive:function(){return h}}},Tc=function(h,a){var b,c=a,d=function(a){b||
...
c:a&&q.length&&(d=q.split(b),e=1<d.length)&&(e=d[1].split("&")[0],d=[d[0],e].join(b),q=q.replace(d,d+c))},fullDom:c,hostname:e,sub:w,master:l,setParam:function(a,b){t[a]=t[a]||{};"string"===typeof b||"number"===typeof b?t[a]=b:g(t[a]).mixin(b)},path:function(a){m=a},toString:function(){var a=w()?w()+".":"",b=m?"/"+m:"",c;q||!g(t).isEmpty()?(c=g(t).isEmpty()?"":g(t).stringify(C,"&"),c="?"+q+(q&&c?"&"+c:c)):c="";return d+"://"+a+l()+b+c}}},sb=function(){var h=function(a){var b=[];g(a).each(function(a,
...
k._onAPIResult(a)}catch(e){h.add(l.ERROR_CODES.BAPI_CALLBACK),b.trigger("sendDiag","bapiClient")}}}},Kb=function(){return{createInstance:function(h,a,b,c){return v.isDomless()?new Uc:new Vc}}},Vc=function(){var h=l.BROWSERS,a=function(a){return b()===a},b=function(){var a="u",b=r.getWindow();try{g.isDef(b.opera)&&g.isDef(b.opera.buildNumber)?a=h.OPERA:g.isDef(b.mozInnerScreenY)?a=h.GECKO:g.isDef(b.chrome)&&g.isDef(b.chrome.csi)?a=h.CHROME:g.isDef(b.msWriteProfilerMark)&&g.isDef(b.crypto)?a=h.MSEDGE:
