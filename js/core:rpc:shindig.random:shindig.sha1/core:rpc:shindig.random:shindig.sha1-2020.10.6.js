...
*/
gapi.util.getOrigin=function(b){var a=b;if(a){a=a.split("#")[0].split("?")[0];a=a.toLowerCase();0==a.indexOf("//")&&(a=window.location.protocol+a);/^[\w\-]*:\/\//.test(a)||(a=window.location.href);b=a.substring(a.indexOf("://")+3);var c=b.indexOf("/");-1!=c&&(b=b.substring(0,c));a=a.substring(0,a.indexOf("://"));if("http"!==a&&"https"!==a&&"chrome-extension"!==a&&"moz-extension"!==a&&"file"!==a&&"android-app"!==a&&"chrome-search"!==a&&"chrome-untrusted"!==a&&"chrome"!==a&&"app"!==a&&"devtools"!==
a)throw Error("Invalid URI scheme in origin: "+a);c="";var d=b.indexOf(":");if(-1!=d){var e=b.substring(d+1);b=b.substring(0,d);if("http"===a&&"80"!==e||"https"===a&&"443"!==e)c=":"+e}b=a+"://"+b+c}else b="";return b};
;
...
gapi.config={};gapi.config.GLOBAL_CFG_KEY_="___gcfg";gapi.config.GLOBAL_CFG_USED_="___gu";gapi.config.JSL_KEY_="___jsl";gapi.config.GAPI_OWNED_CONFIG_KEY_="___goc";gapi.config.DEFAULT_CFG_KEY_="cd";gapi.config.USER_CFG_KEY_="cu";gapi.config.INJECTED_CFG_KEY_="ci";gapi.config.SAVED_CFG_KEY_="cfg";gapi.config.ALL_SOURCES_KEY_="us";gapi.config.ATTR_PROCESSED_CONFIG_="gapi_processed";gapi.config.jsl_=function(){return window[gapi.config.JSL_KEY_]=window[gapi.config.JSL_KEY_]||{}};
gapi.config.jslComponent_=function(a){var b=gapi.config.jsl_();b[a]=b[a]||[];return b[a]};gapi.config.cfg_=function(a){var b=gapi.config.jsl_();b[gapi.config.SAVED_CFG_KEY_]=!a&&b[gapi.config.SAVED_CFG_KEY_]||{};return b[gapi.config.SAVED_CFG_KEY_]};gapi.config.isArray_=function(a){return"object"===typeof a&&/\[native code\]/.test(a.push)};
gapi.config.foldConfig_=function(a,b,d){if(b&&"object"===typeof b)for(var c in b)!Object.prototype.hasOwnProperty.call(b,c)||d&&c===gapi.config.GAPI_OWNED_CONFIG_KEY_&&"undefined"===typeof b[c]||(a[c]&&b[c]&&"object"===typeof a[c]&&"object"===typeof b[c]&&!gapi.config.isArray_(a[c])&&!gapi.config.isArray_(b[c])?gapi.config.foldConfig_(a[c],b[c]):b[c]&&"object"===typeof b[c]?(a[c]=gapi.config.isArray_(b[c])?[]:{},gapi.config.foldConfig_(a[c],b[c])):a[c]=b[c])};
gapi.config.getSelfScriptTags_=function(){var a=document.scripts||document.getElementsByTagName("script")||[],b=[],d=[];d.push.apply(d,gapi.config.getAllSourceUrls_());for(var c=0;c<a.length;++c)for(var e=a[c],f=0;f<d.length;++f)e.src&&0==e.src.indexOf(d[f])&&b.push(e);0==b.length&&0<a.length&&a[a.length-1].src&&b.push(a[a.length-1]);return b};
gapi.config.appendInlineConfig_=function(){for(var a=gapi.config.jslComponent_(gapi.config.USER_CFG_KEY_),b=gapi.config.getSelfScriptTags_(),d=0;d<b.length;++d)if(!b[d].getAttribute(gapi.config.ATTR_PROCESSED_CONFIG_)){b[d].setAttribute(gapi.config.ATTR_PROCESSED_CONFIG_,!0);var c=gapi.config.getInnerText_(b[d]);(c=gapi.config.parseConfig_(c))&&a.push(c)}};gapi.config.getAllSourceUrls_=function(){return gapi.config.jslComponent_(gapi.config.ALL_SOURCES_KEY_)};
gapi.config.getInnerText_=function(a){if(a){var b=a.nodeType;return 3==b||4==b?a.nodeValue:a.textContent||a.innerText||a.innerHTML||""}};
gapi.config.parseConfig_=function(a){if(a&&!/^\s+$/.test(a)){for(;0==a.charCodeAt(a.length-1);)a=a.substring(0,a.length-1);try{var b=window.JSON.parse(a)}catch(d){}if("object"===typeof b)return b;try{b=(new Function("return ("+a+"\n)"))()}catch(d){}if("object"===typeof b)return b;try{b=(new Function("return ({"+a+"\n})"))()}catch(d){}return"object"===typeof b?b:{}}};
gapi.config.mergeFragment_=function(a,b){var d={};d[gapi.config.GAPI_OWNED_CONFIG_KEY_]=void 0;a.length&&a[a.length-1]&&Object.hasOwnProperty.call(a[a.length-1],gapi.config.GAPI_OWNED_CONFIG_KEY_)&&"undefined"===typeof a[a.length-1][gapi.config.GAPI_OWNED_CONFIG_KEY_]&&(d=a.pop());gapi.config.foldConfig_(d,b);a.push(d)};
...
a=0;for(b=c.length;a<b;++a)gapi.config.foldConfig_(gapi.config.cfg_(),c[a],!0);a=0;for(b=d.length;a<b;++a)gapi.config.foldConfig_(gapi.config.cfg_(),d[a],!0)};gapi.config.get=function(a,b){var d=gapi.config.cfg_();if(!a)return d;a=a.split("/");for(var c=0,e=a.length;d&&"object"===typeof d&&c<e;++c)d=d[a[c]];return c===a.length&&void 0!==d?d:b};
gapi.config.update=function(a,b){var d;if("string"===typeof a){var c=d={};a=a.split("/");for(var e=0,f=a.length;e<f-1;++e){var g={};c=c[a[e]]=g}c[a[e]]=b}else d=a;gapi.config.mergeConfig_(d)};gapi.config.init=function(){gapi.config.mergeConfig_()};gapi.config.legacyInit&&gapi.config.legacyInit();gapi.config.init();
;
