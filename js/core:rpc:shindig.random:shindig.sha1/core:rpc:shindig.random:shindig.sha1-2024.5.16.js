...
$jscomp.polyfill("String.prototype.endsWith",function(a){return a?a:a=function(b,d){var c=$jscomp.checkStringArgs(this,b,"endsWith");b+="";void 0===d&&(d=c.length);d=Math.max(0,Math.min(d|0,c.length));for(var e=b.length;0<e&&0<d;)if(c[--d]!=b[--e])return!1;return 0>=e}},"es6","es3");gapi.config={};gapi.config.GLOBAL_CFG_KEY_="___gcfg";gapi.config.GLOBAL_CFG_USED_="___gu";gapi.config.JSL_KEY_="___jsl";gapi.config.GAPI_OWNED_CONFIG_KEY_="___goc";gapi.config.DEFAULT_CFG_KEY_="cd";
gapi.config.USER_CFG_KEY_="cu";gapi.config.INJECTED_CFG_KEY_="ci";gapi.config.SAVED_CFG_KEY_="cfg";gapi.config.ALL_SOURCES_KEY_="us";gapi.config.ATTR_PROCESSED_CONFIG_="gapi_processed";gapi.config.jsl_=function(){return window[gapi.config.JSL_KEY_]=window[gapi.config.JSL_KEY_]||{}};gapi.config.jslComponent_=function(a){var b=gapi.config.jsl_();b[a]=b[a]||[];return b[a]};gapi.config.cfg_=function(a){var b=gapi.config.jsl_();b[gapi.config.SAVED_CFG_KEY_]=!a&&b[gapi.config.SAVED_CFG_KEY_]||{};return b[gapi.config.SAVED_CFG_KEY_]};
gapi.config.isArray_=function(a){return"object"===typeof a&&/\[native code\]/.test(a.push)};
...
gapi.config.getSelfScriptTags_=function(){var a=document.getElementsByTagName("script")||[],b=[],d=[];d.push.apply(d,gapi.config.getAllSourceUrls_());for(var c=0;c<a.length;++c)for(var e=a[c],f=0;f<d.length;++f)e.src&&0==e.src.indexOf(d[f])&&b.push(e);0==b.length&&0<a.length&&a[a.length-1].src&&b.push(a[a.length-1]);return b};
...
gapi.config.getInnerText_=function(a){if(a){var b=a.nodeType;return 3==b||4==b?a.nodeValue:a.textContent||""}};
gapi.config.parseConfig_=function(a,b){if(a&&!/^\s+$/.test(a)){for(;0==a.charCodeAt(a.length-1);)a=a.substring(0,a.length-1);var d=""+a;try{var c=window.JSON.parse(a)}catch(k){}if("object"===typeof c)return c;try{c=window.JSON.parse("{"+a+"}")}catch(k){}if("object"===typeof c)return c;var e=RegExp("([^\"',{}\\s]+?)\\s*:\\s*(.*?)[,}\\s]","g"),f=function(k,l,h){h=h.startsWith('"')?"%DOUBLE_QUOTE%"+h.substring(1):h;h=h.endsWith('"')?h.slice(0,-1)+"%DOUBLE_QUOTE%":h;return"%DOUBLE_QUOTE%"+l+"%DOUBLE_QUOTE%:"+
h};a=a.replace(e,f);a=a.replace(/\\'/g,"%SINGLE_QUOTE%");a=a.replace(/"/g,'\\"');a=a.replace(/'/g,'"');a=a.replace(/%SINGLE_QUOTE%/g,"'");a=a.replace(/%DOUBLE_QUOTE%/g,'"');try{c=window.JSON.parse(a)}catch(k){}if("object"===typeof c)return c;try{c=window.JSON.parse("{"+a+"}")}catch(k){}if("object"===typeof c)return c;a=document.getElementsByTagName("script")||[];var g;0<a.length&&(g=a[0].nonce||a[0].getAttribute("nonce"));if(g&&g!==b)return{};c=gapi.config.evalConfigText_(d);return"object"===typeof c?
c:{}}};gapi.config.evalConfigText_=function(a){try{var b=(new Function("return ("+a+"\n)"))()}catch(d){}if("object"===typeof b)return b;try{b=(new Function("return ({"+a+"\n})"))()}catch(d){}return b};
