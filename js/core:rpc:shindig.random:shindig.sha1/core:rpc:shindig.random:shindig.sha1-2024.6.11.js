...
h};a=a.replace(e,f);a=a.replace(/\\'/g,"%SINGLE_QUOTE%");a=a.replace(/"/g,'\\"');a=a.replace(/'/g,'"');a=a.replace(/%SINGLE_QUOTE%/g,"'");a=a.replace(/%DOUBLE_QUOTE%/g,'"');try{c=window.JSON.parse(a)}catch(k){}if("object"===typeof c)return c;try{c=window.JSON.parse("{"+a+"}")}catch(k){}if("object"===typeof c)return c;a=document.getElementsByTagName("script")||[];var g;0<a.length&&(g=a[0].nonce||a[0].getAttribute("nonce"));if(g&&g===b||!g&&gapi.config.isGoogleDomain_(window.location.hostname))if(c=
gapi.config.evalConfigText_(d),"object"===typeof c)return c;return{}}};
