...
(function() {
    ...
    //lview加载时长
    var lviewtime = '';
    if (!nocrystal &&  crystal.cps && crystal.cps.tagMap) {
    ...
    if(bPlatform === 'qqvideoclient') {
      ...
      //先统计下host，再统计下浏览器分布
      g_btrace_dp3.src = "//dp3.qq.com/stdlog?bid=crystal&pid=visit&host="+encodeURIComponent(host)+"&cv="+cver+"&ref="+encodeURIComponent(ref)+"&ua="+browse+"&from="+from+"&val2="+encodeURIComponent(url)+"&sw="+savailWidth+"&sh="+savailHeight+"&ww="+pwidth+"&wh="+pheight+"&time1="+lviewtime;
      //针对aio计算停留时间，因为本来这个监测文件已经延迟10s加载了，所以我们监测3个点位：10s，60s，300s
      var reportStay = function(step) {
        var im = new Image(1,1);
        im.src = "//dp3.qq.com/stdlog?bid=crystal&host="+encodeURIComponent(host)+"&pid=aiostay&ua="+browse+"&from="+from+"&val1="+step;
      }
      setTimeout(function() {
          reportStay(1);
      },10*1000);
